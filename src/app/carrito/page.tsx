...
const CartPage = () => {
    ...
    /* useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                throw new Error(
                    "No hay usuario logueado, se usará localStorage para el carrito."
                );
            }
        };
    ...
    useEffect(() => {
        const fetchCart = async () => {
            try {
                ...
                const response = await fetch(`${CART_API}/${user.userId}`);
                if (!response.ok)
                    ...
                    return {
                        ...
                        image:
                            productData.images.length > 0
                                ...
                                : "/img/default-product.jpg", // 🔥 Accede al primer elemento de images
    ...
    const addToCart = (product: CartItem) => {
        ...
        if (user) {
            ...
            fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/cart/item/${product.id}`,
                {
                    method: "POST",
    ...
    const handleRemoveItem = async (productId: string) => {
        ...
        try {
            ...
            const responseData = await response.json();
            console.log(
                "Respuesta del servidor (producto eliminado):",
                responseData
            );
    ...
    const handleCheckout = async () => {
        ...
        try {
            const res = await checkoutService(user.userId);
