...
const page = () => {
  ...
  return (
    <section className="relative">
      {modal && <AddEditProductModal setModal={setModal} product={productEdit} isEditing={modalEdit} />}
      {deleteModal && <DeleteModal setDeleteModal={setDeleteModal} modalText="¿Estás seguro de que deseas borrar este producto?" itemId={productEdit?.id || ''} />}
        ...
        <section className="flex flex-col gap-1">
          ...
          {loading ? (
            ...
            products.map((item: Product, i) => (
              <article key={i} className="bg-white overflow-hidden rounded-md border border-neutral-200 flex flex-col">
                <div className="flex">
                  <div className="flex w-full">
                    <div className={`${commonStyle} w-2/12`}>
                    ...
                    <div className={`${commonStyle} w-1/12 text-xl gap-4`}>
                      <button onClick={() => { setProductEdit(item); setModal(true); setModalEdit(true); }}>
                        <FaEdit />
                      </button>
                      <button onClick={() => { setProductEdit(item); setDeleteModal(true); }}>
